...
class PasswordEncoderSHA256Test {
    ...
    @Test
    @DisplayName("[성공] 비밀번호를 SHA-256으로 인코딩하면 올바른 해시 값을 생성")
    void encode_Success() {
        // given
        ...
        // then
        assertThat(encodedPassword).isNotNull().hasSize(64);
    }

    @Test
    @DisplayName("[성공] 인코딩된 비밀번호와 원본 비밀번호가 일치하면 true 반환")
    ...
    @Test
    @DisplayName("[실패] 인코딩된 비밀번호와 원본 비밀번호가 일치하지 않으면 false 반환")
    void matches_Fail() {
        // given
        CharSequence rawPassword = "ValidPassword1!";
    ...
    @Test
    @DisplayName("[예외] 지원하지 않는 알고리즘을 사용할 때 EnergyException 발생")
    void encode_UnsupportedAlgorithm() {
        ...
        try {
            ...
            // when & then
            EnergyException exception = assertThrows(EnergyException.class,
